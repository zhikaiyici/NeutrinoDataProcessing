function IBDEvent = IBDSelection_PANDA(event)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%          |         瞬时信号       |           延迟信号
% 甄别项目 |        甄别条件        |           甄别条件
% 预甄别   |     Eth > 200 keV      |        Eth > 200 keV
% Ntrigger |    2 ≤ Ntrigger ≤ 16   |    2 ≤ Ntrigger ≤ 16
% Etotal   | 3 MeV < Etotal ≤ 6 MeV | 2.5 MeV < Etotal ≤ 8 MeV
...
dropedEvent(index) = 0;
event(dropedEvent,:) = []; % 去时间差大于300us或小于8us的除相邻事件
...
edep = reshape(edep',[16, numel(edep) ./ 16])';
edepSorted = sort(edep, 2, 'descend');
% dropedEvent = (sum(edepSorted, 2) < 2500 |edepSorted(:,1) > 7000 | edepSorted(:,2) > 3000 | edepSorted(:,3) > 2000 | edepSorted(:,4) > 1000);
...
delayedEvent(dropedEvent,:) = []; % 去除Etotal < 2.5MeV、E1st > 7MeV、E2nd > 3MeV、E3rd > 2 MeV、E4th > 1MeV的延迟事件
